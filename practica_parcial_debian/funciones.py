...
def cargar_bailando(matriz:list[list])->list[list]:
    """esta mal esta funcion, incluye el encabezado adentro no usar
...
def encontrar_numero(matriz: list[list], numero:int, columna:int)->None:
    ...
    if bandera == 1:
        imprimir_matriz_con_encabezado(matrix)
    else:    
...
def mostrar_ganador_y_empates(matriz: list[list], columna_uno: int, columna_dos: int, operador) -> None:
    ...
    if empates:
        ...
        print('EMPATADOS LOS PARTICIPANTES')
        imprimir_matriz_con_encabezado(empates)
    else:
        print('EMPATADOS LOS PARTICIPANTES')
...
def calcular_promedio_bailarines(jurado_uno:int,jurado_dos:int,jurado_tres:int)->float:
        suma = jurado_uno + jurado_dos + jurado_tres
        nota_promedio = suma / 3
